...
export default function MazokuBloodstone() {
  ...
  const [focusedBloodstone, setFocusedBloodstone] = useState<boolean>(false);
  const [focusedRatio, setFocusedRatio] = useState<boolean>(false);
  const bloodstoneInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleBloodstoneAmountChange = (
    ...
    if (!isNaN(numericValue) && numericValue >= 0) {
      setBloodstoneAmount(numericValue);
    }
  };
  ...
  const handleCalculate = () => {
    if (bloodstoneAmount > 0 && ratio > 0) {
      const calculatedAmount = parseFloat(
        (bloodstoneAmount / ratio).toFixed(2)
      );
  ...
  const handleRestart = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => bloodstoneInputRef.current?.focus(), 50);
    }, 400);
  };
  ...
  const handleCopy = async () => {
    if (!result) return;
    const text = `\`\`\`css
Bloodstone Amount:     ${bloodstoneAmount} ${
  ...
  return (
    <PageWrapper>
      <div className="min-h-screen flex items-center justify-center p-4">
        <TopRightMenu currency={"inr"} href={"/mazoku-bloodstone-inr"} />
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-2 font-mono text-[11px] tracking-[0.12em] uppercase text-white/30">
                <span className="w-1.5 h-1.5 rounded-full bg-yellow-500 shadow-[0_0_8px_#eab308] shrink-0" />
                Mazoku · Bloodstone Calculator
              </div>

              {/* Bloodstone Amount Input */}
              <div className="w-full">
                ...
                >
                  ...
                  <input
                    ...
                    onFocus={() => setFocusedBloodstone(true)}
                    onBlur={() => setFocusedBloodstone(false)}
              ...
              <div className="w-full">
                <div
                  className={`flex items-center rounded-[14px] bg-white/[0.04] border transition-colors duration-200 ${
                    focusedRatio ? "border-yellow-500/60" : "border-white/10"
                  }`}
                >
                  <span className="px-5 h-16 flex items-center font-mono text-[13px] font-medium text-white/30 border-r border-white/[0.08] bg-black/20 select-none whitespace-nowrap shrink-0">
                    Ratio
              ...
              <button
                ...
                disabled={bloodstoneAmount === 0 || ratio === 0}
                className="w-full h-[52px] bg-yellow-500 rounded-xl font-sans text-[15px] font-bold tracking-[0.02em] text-white cursor-pointer flex items-center justify-center gap-2.5 transition-all duration-150 hover:opacity-85 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:opacity-50 disabled:active:scale-100"
              >
                Calculate
                <ArrowRight size={16} />
              </button>
              ...
              <span className="font-mono text-[11px] text-white/20 tracking-[0.06em]">
                {ratio} bloodstones = $1 · Including PayPal fee
              </span>
            </div>
              ...
              {result && (
                <>
                  ...
                  <p className="font-mono text-center w-full text-[13px] text-white/45 leading-[1.8] italic">
                    You are selling{" "}
                    <strong className="text-yellow-400 not-italic">
                      {bloodstoneAmount}{" "}
                      {bloodstoneAmount === 1 ? "Bloodstone" : "Bloodstones"}
                    </strong>{" "}
                    at{" "}
                  ...
                  <div className="w-full rounded-[14px] border border-white/[0.07] overflow-hidden">
                    ...
                    {/* Highlighted row */}
                    <div className="flex justify-between items-center px-[18px] py-[14px] bg-white border-t border-yellow-500/[0.15]">
