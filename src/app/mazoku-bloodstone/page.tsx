...
export default function SofiWist() {
  const [bloodstoneAmount, setBloodstoneAmount] = useState<number>(45000);
  const [ratio, setRatio] = useState<number>(7000);
  ...
  const handleCalculate = () => {
    const calculatedAmount = parseFloat((bloodstoneAmount / ratio).toFixed(2));
...
-# Calculated Using: [PayPal | Sofi | Karuta | Mazoku Fee Calculator by ItsMe Prince]( https://paypal-and-sofi-wist-fee-calculator.vercel.app/)`;
  ...
  return (
    <div className="h-screen flex flex-col justify-center items-center text-yellow-950">
      <HomeButton />
      {popupMessage && (
        <div className="absolute top-5 bg-yellow-500 text-yellow-950 px-4 py-2 rounded-md shadow-lg transition-all ease-in-out duration-1000">
          {popupMessage}
        </div>
      )}
      <div
      ...
      >
        {/* Front Side */}
        <div
          className={`absolute w-full h-full bg-gradient-to-r from-yellow-600 to-yellow-400 rounded-2xl shadow-yellow-600/50 shadow-2xl p-5 backface-hidden flex flex-col justify-center items-center gap-5 ${
            isFlipped ? "hidden" : ""
          }`}
          style={{
        ...
        >
          <h1 className="text-xl font-semibold text-yellow-950 z-10">
            Enter Bloodstone Amount ...
          </h1>
          <div className="relative flex justify-center items-center shadow-md shadow-yellow-600/60 rounded-md">
            ...
            <input
              ...
              className={`z-10 antialiased text-center text-4xl p-2 rounded-r-md text-black w-[250px] focus:outline-none placeholder:text-2xl ${
                isInputDisabled ? "bg-yellow-950 cursor-not-allowed" : ""
              }`}
          ...
          <div className="relative flex justify-center items-center shadow-md shadow-yellow-600/60 rounded-md">
            <input
              ...
              className={`z-10 antialiased text-center text-4xl p-2 rounded-l-md text-yellow-950 w-[150px] focus:outline-none ${
                isInputDisabled ? "bg-yellow-950 cursor-not-allowed" : ""
              }`}
              disabled={isInputDisabled}
            />
          ...
          <div className="flex gap-5 mt-5">
            <button
              ...
              className="z-10 bg-white rounded-md px-5 h-[45px] text-yellow-950 shadow-yellow-700/80 hover:shadow-yellow-800 shadow-xl hover:scale-105 transition-all ease-in-out duration-200 flex justify-center items-center"
            >
              Calculate
            </button>
            <Link href={"/mazoku-bloodstone-inr"}>
              <button className="z-10 bg-white rounded-md px-5 h-[45px] text-yellow-950 shadow-yellow-700/80 hover:shadow-yellow-800 shadow-xl hover:scale-105 transition-all ease-in-out duration-200 flex justify-center items-center">
                Switch to INR
              </button>
            </Link>
          </div>
        ...
        >
          ...
          <div className="flex gap-4">
            ...
            <button
              onClick={handleCopy}
              className="z-10 bg-white rounded-md px-5 h-[45px] text-yellow-950 shadow-yellow-700/80 hover:shadow-yellow-800 shadow-xl hover:scale-105 transition-all ease-in-out duration-200 flex justify-center items-center gap-2 active:scale-125"
            >
              Copy
