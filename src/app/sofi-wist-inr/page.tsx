...
export default function SofiWistINR() {
  ...
  const [focusedWist, setFocusedWist] = useState<boolean>(false);
  ...
  const handleCopy = async () => {
    if (!inrAmount) return;
    const text = `\`\`\`css
Wist Amount:         ${wistAmount} ${wistAmount === 1 ? "Wist" : "Wists"}
  ...
  return (
    <PageWrapper>
      <div className="min-h-screen flex items-center justify-center p-4">
        <TopRightMenu currency={"usd"} href={"/sofi-wist"} />
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-2 font-mono text-[11px] tracking-[0.12em] uppercase text-white/30">
                <span className="w-1.5 h-1.5 rounded-full bg-pink-500 shadow-[0_0_8px_#10b981] shrink-0" />
                Sofi · Wist Calculator (INR)
              </div>
              ...
              <span className="font-mono text-[11px] text-white/20 tracking-[0.06em]">
                ₹{ratio.toFixed(2)} per wist · INR Calculator
              </span>
            </div>
